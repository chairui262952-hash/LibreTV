...
function addVersionInfoToFooter() {
    checkForUpdates().then(result => {
        ...
        // 显示版本号，不显示日期
        versionElement.innerHTML = `v${result.current}`;
        
        // 如果有更新，添加更新提示
        if (result.hasUpdate) {
            versionElement.innerHTML += ` <span class="inline-flex items-center bg-[#F4D8D8] text-[#5F5449] text-xs px-2 py-0.5 rounded-full ml-1 cursor-pointer font-medium" style="animation:pulse 2s ease-in-out infinite;">
                发现新版 v${result.latest}
            </span>`;
            ...
            // 最新版本
            versionElement.innerHTML = `v${result.current} <span class="text-xs text-[#9B8E82]">· 已是最新</span>`;
        }
